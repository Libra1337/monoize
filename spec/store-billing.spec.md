...
SB-UI-17. Each Alipay, WeChat Pay, or Stripe Channel row MUST expose a Readiness action. An HTTP Adapter row MUST NOT expose that action. The Readiness dialog MUST load SB-C-35 through SWR and render a skeleton before data arrives. It MUST constrain currency and checkout-action controls to the selected adapter, expose every client-supplied SB-C-35 field, and MUST NOT expose server-bound credential or Admin identifiers as editable fields. It MUST NOT send a request unless the metadata, amount limits, adapter constraints, and validity period satisfy SB-C-28 and SB-C-35. A successful save MUST optimistically replace the SWR readiness value, roll back on error, apply the server profile, and revalidate the Payment Channel list so effective availability updates without a manual refresh.

SB-UI-18. The Store Management Payment Channels child page MUST expose Retention as a modal dialog. It MUST NOT add another Store Management child page. The Retention dialog MUST load SB-PR-14A through SWR and render a skeleton before data arrives. It MUST show the consecutive failure count, the checkout pause state, the retention run count, and the legal hold count. It MUST expose a current-password field, a reason field, and a run action. The run action MUST obtain a `retention_operation` reauthentication grant, call SB-PR-14B, clear the password and reason fields on success, and revalidate the SB-PR-14A value. The dialog MUST render an evidence-digest field and a containment action if and only if `checkout_paused = true`. The containment action MUST obtain a `retention_operation` grant, call SB-PR-14C, clear the password, reason, and digest fields on success, and revalidate the SB-PR-14A value. It MUST NOT send a request unless the reason is nonempty after trimming, and, for containment, the trimmed digest contains exactly 64 characters. The dialog MUST NOT create, extend, or display individual legal holds.
