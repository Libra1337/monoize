...
C3-i-a. An OpenAI Chat Completions-shaped usage object under C3-i MAY report the cached subset of its inclusive prompt total in a non-standard wire field at the top level of the usage object. Decode MUST resolve `cache_read_tokens` from the first present field with a value greater than zero, in this precedence order: `prompt_tokens_details.cached_tokens`, `input_tokens_details.cached_tokens`, `prompt_cache_hit_tokens` (DeepSeek), `input_cache_read` (DashScope-compatible), `cache_read_input_tokens` (an Anthropic-style field emitted by a chat-shaped endpoint). Decode MUST NOT modify `input_tokens` when mapping any of these aliases, because the wire prompt total already includes the cached subset. This resolution MUST apply identically to the streaming and non-streaming decode paths. Unrecognized wire fields MUST remain preserved in `usage.extra_body`.
