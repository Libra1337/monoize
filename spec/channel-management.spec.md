...
CP-INV-16a. Validation at Channel write time cannot bind a hostname to the address it will reach at dispatch time. Therefore, unless `MONOIZE_ALLOW_PRIVATE_UPSTREAM=1` is set, Monoize MUST re-evaluate the target host immediately before every upstream dispatch, for both the JSON dispatch path and the multipart dispatch path, and MUST reject the dispatch when any resolved address is in a range named in CP-INV-16. A literal IP host MUST be classified directly without resolution. A hostname that fails to resolve MUST NOT be rejected by this check; the dispatch proceeds and fails later as an ordinary upstream network error. The classification result MAY be cached per host and port for at most 30 seconds. A rejected dispatch MUST NOT be reported as an upstream HTTP status.
