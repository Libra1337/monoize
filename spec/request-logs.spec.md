...
RL1f. On ordinary Primary startup after local Store lease acquisition, all request-log rows with `status = "pending"` MUST be transitioned to `status = "error"` with `error_code = "server_shutdown"` and `error_message = "interrupted by server restart"`. This cleanup MUST execute before the HTTP listener begins accepting connections. A Primary with `MONOIZE_BOOT_STANDBY_LEASE=1` MUST skip global pending-log cleanup at startup and lease acquisition because overlapping processes can own live requests.

RL1g. On receipt of SIGINT or SIGTERM, the server MUST initiate graceful shutdown: set the process-local background-shutdown flag, stop accepting new connections, allow in-flight requests to drain, wait for all tracked request-log and active-probe work to finish, flush all write batchers (including the request-log batcher), then transition any remaining `"pending"` rows (legacy) to `"error"` with the same fields as RL1f before process exit. Global pending-log cleanup MUST run only on a Primary with a valid local Store lease and `MONOIZE_BOOT_STANDBY_LEASE` not equal to `1`.
...
RL18a. When a successful active probe returns both prompt and completion token counts, its charge calculation MUST read `billing_rate_records`; it MUST NOT read token prices from `model_metadata_records`. It MUST use the mapping's effective Profile and multiplier from `provider-pricing.spec.md` section 7. It MUST normalize and try the redirected upstream model before the logical model. It MUST NOT select a Profile from `pricing_profile_model_patterns` or `models_dev_provider`.
...
FL7a. The filter-control area MUST display the total charge sum for the current filter conditions. The displayed value MUST come from the most recent REST response field `total_charge_nano_usd` for the active filters. It MUST include every matching persisted row and MUST be independent of request `limit`, request `offset`, and the number of rows currently loaded by infinite scrolling. The frontend MUST NOT compute this value by summing `loadedLogs` or the current response page. The value MUST follow the Console display currency (DL3h of `dashboard-ui-layout.spec.md`): USD renders as regular USD currency with 6 fractional digits (e.g. `$1.234567`); CNY renders through the shared nano-USD conversion helper with the loaded `cny_per_usd` snapshot and 6 fractional digits under exact BigInt conversion, because a per-request charge is routinely below ¥0.01, falling back to the USD rendering while the snapshot is unavailable. The label MUST use the i18n key `requestLogs.totalCost`. The element MUST be displayed in the summary area (top-right) alongside the existing "Showing X-Y of Z" text.
...
FL25. The `charge_nano_usd` (Cost) column displayed value MUST follow the Console display currency (DL3h of `dashboard-ui-layout.spec.md`): USD renders as regular USD currency with exactly 6 fractional digits (for example: `$0.000123`); CNY renders through the shared nano-USD conversion helper with the loaded `cny_per_usd` snapshot and 6 fractional digits under exact BigInt conversion, because a per-request charge is routinely below ¥0.01, falling back to the USD rendering while the snapshot is unavailable. It MUST NOT use threshold shorthand (for example: `<$0.0001`). Formatting MUST operate on the integer string with `BigInt`; the USD path MUST round to the nearest micro-dollar with ties away from zero and MUST NOT pass through JavaScript `Number`, `parseFloat`, or `Intl.NumberFormat`. The same currency rule applies to every other rendered charge derived from `charge_nano_usd` on this page, including the FL26 breakdown tooltip subtotals and the expanded-detail charges.
...
FL27d. The visible Input cell MUST always render the total input count selected by FL27a; it MUST NOT substitute the uncached-input count. When the cached-input count selected from `usage_breakdown_json.input.cached_tokens` or scalar `cache_read_tokens` is positive and total input is positive, the cell MUST use exactly two non-wrapping lines and the second line MUST render `<formatted cached count> / <percentage>%`, where `percentage = round(cached input / total input * 10) / 10` and trailing `.0` MUST be omitted. For example, total input `32000` and cached input `16000` render as first line `32,000` and second line `16,000 / 50%`; total input `314241` and cached input `313088` render `313,088 / 99.6%`. The second line MUST use smaller muted text and MUST NOT use a success/status color. When cached input is absent or not positive, or total input is absent or not positive, the cell MUST render only the total-input line and vertically center that line within the 44-pixel body row. It MUST NOT reserve an empty second line or render `0`, `-`, or a percentage. The complete visible cell remains the FL27 hover surface.
