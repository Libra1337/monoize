...
- During that merge, a missing field or an empty string in one source MAY be filled from a non-empty value in another source. Except for the message boundary-whitespace, terminal reasoning-summary, and terminal encrypted-content rules below, differing non-empty typed semantic values for the same slot MUST fail reconciliation.
