...
mod tests {
    ...
    /// Key-less request-log rows (Playground traffic carries `api_key_id IS NULL`)
    /// must not break the per-key daily-spend aggregate that the /tokens list
    /// enriches every key with; a NULL group row used to fail the non-null
    /// `key_id` decode and 500 the whole key list for anyone who had used the
    /// Playground since the UTC day started.
    #[tokio::test]
    async fn user_api_key_daily_spend_ignores_keyless_playground_rows() {
        use sea_orm_migration::MigratorTrait;
        let db = crate::db::DbPool::connect("sqlite::memory:")
            .await
            .expect("db connects");
        ...
        let store = super::UserStore::new(db, log_tx)
            ...
            .expect("store creates");
        let user = store
            .create_user("player", "password123", crate::users::UserRole::User, None)
            .await
            .expect("user creates");
        let (key, _) = store
            .create_api_key_extended(
        ...
        let now_ms = chrono::Utc::now().timestamp_millis();
        {
            ...
            write
                .execute(
                    store.db.stmt(
                        "INSERT INTO request_logs (id, request_id, created_at, created_at_unix_ms, model, upstream_model,
                            charge_nano_usd, user_id, api_key_id, input_tokens, cache_read_tokens, output_tokens, status, request_kind)
                         VALUES ($1, 'r1', $4, $2, 'm', '', '1000000000', $3, $5, 1, 0, 1, 'success', 'playground')",
                        vec![
                            "log-keyed".into(),
                            now_ms.into(),
                            user.id.clone().into(),
                ...
                .await
                .expect("keyed log row inserts");
            write
                .execute(
                    store.db.stmt(
                        "INSERT INTO request_logs (id, request_id, created_at, created_at_unix_ms, model, upstream_model,
                            charge_nano_usd, user_id, input_tokens, cache_read_tokens, output_tokens, status, request_kind)
                         VALUES ($1, 'r2', $4, $2, 'm', '', '9000000000', $3, 1, 0, 1, 'success', 'playground')",
                        vec![
                            "log-keyless".into(),
                            now_ms.into(),
                            user.id.clone().into(),
                ...
                .await
                .expect("keyless log row inserts");
        }
        let spend = store
            .get_user_api_keys_daily_spend(&user.id)
            .await
            .expect("daily spend ignores keyless rows");
        assert_eq!(spend.len(), 1);
        assert_eq!(spend.get(&key.id).map(String::as_str), Some("1000000000"));
    }
...
impl UserStore {
    ...
    pub async fn get_user_api_keys_daily_spend(
        ...
        // Key-less rows (Playground traffic) have no key to attribute spend
        // to and would fail the non-null `key_id` decode below.
        let sql = format!(
            "SELECT rl.api_key_id AS key_id, {}              FROM request_logs rl              WHERE rl.user_id = $1 AND rl.created_at_unix_ms >= $2              AND rl.api_key_id IS NOT NULL              GROUP BY rl.api_key_id",
            charge_aggregate_columns(is_postgres)
        );
        let rows = self
            .db
