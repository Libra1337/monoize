...
use super::adapters::epay::{
    EpayCallbackError, EpayCredential, verify_payment_callback as verify_epay_payment_callback,
};
...
const CALLBACK_QUERY_MAX_BYTES: usize = 8_192;
...
pub async fn store_payment_callback(
    ...
    if adapter_kind == "epay" {
        if query.len() > CALLBACK_QUERY_MAX_BYTES {
            return Err(AppError::new(
    ...
    if adapter_kind != "stripe" || method != axum::http::Method::POST {
        return Err(AppError::new(
            StatusCode::SERVICE_UNAVAILABLE,
...
async fn handle_epay_callback(
    ...
    let credentials = load_callback_credentials(state, channel_id, "epay").await?;
    if credentials.is_empty() {
        return Err(AppError::new(
            StatusCode::NOT_FOUND,
            "payment_channel_not_found",
    ...
    for stored in credentials {
        ...
        let Ok(credential) = EpayCredential::from_json(&plaintext) else {
            continue;
        };
        ...
        usable_credential = true;
        match verify_epay_payment_callback(&credential, query) {
            Ok(payment) => {
                selected = Some((stored, payment));
                break;
            }
    ...
    let Some((stored, payment)) = selected else {
        ...
        return Err(invalid_epay_event(
            authenticated_error.unwrap_or(EpayCallbackError::Authentication),
        ));
    };
    let credential_version_id = stored.id;
    let merchant_account_identity = stored.account_identity_digest;
    let binding = load_bound_epay_attempt(
        state,
        channel_id,
    ...
    let body_digest = Sha256::digest(query.as_bytes())
        .iter()
        .map(|byte| format!("{byte:02x}"))
        .collect();
    let parsed_json = json!({
    ...
    let (attempt_id, order_id) = match binding {
        ...
        None => {
            callback_store
                ...
                .await
                .map_err(map_callback_store_error)?;
            return Err(invalid_epay_event(EpayCallbackError::InvalidPaymentEvent));
        }
    };
...
/// Binds one verified EPay callback to exactly one attempt. The expected method participates in
/// the predicate so a callback for one method never fulfills an attempt created for the other.
async fn load_bound_epay_attempt(
    state: &AppState,
    channel_id: &str,
    ...
    let rows = state
        ...
        .query_all(state.db_pool.stmt(
            "SELECT a.id AS attempt_id, a.order_id
             ...
             WHERE a.channel_id = $1 AND a.adapter_kind = 'epay'
               AND a.credential_version_id = $2
               AND a.expected_payment_method = $4
...
fn invalid_epay_event(error: EpayCallbackError) -> AppError {
    AppError::new(
        StatusCode::BAD_REQUEST,
        "invalid_payment_callback",
        "payment callback verification failed",
