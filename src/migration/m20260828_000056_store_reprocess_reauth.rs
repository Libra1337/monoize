...
async fn migrate_scope(manager: &SchemaManager<'_>, include_reprocess: bool) -> Result<(), DbErr> {
    ...
    if backend == DbBackend::Postgres {
        if !include_reprocess {
            tx.execute(Statement::from_string(
                backend,
                "DELETE FROM store_reauth_grants WHERE scope = 'reprocess'".to_string(),
            ))
            .await?;
        }
        tx.execute(Statement::from_string(
        ...
        tx.execute(Statement::from_string(
            ...
            format!(
                ...
                allowed_scopes(include_reprocess)
            ),
        ))
        .await?;
        tx.execute(Statement::from_string(
        ...
        tx.execute(Statement::from_string(
            backend,
            "ALTER TABLE store_reauth_grants RENAME TO store_reauth_grants_v056".to_string(),
        ))
        .await?;
        tx.execute(Statement::from_string(
            backend,
            sqlite_table_sql(include_reprocess),
        ))
        .await?;
        tx.execute(Statement::from_string(
            backend,
            format!(
                "INSERT INTO store_reauth_grants
                 ...
                 FROM store_reauth_grants_v056{}",
                if include_reprocess {
                    ""
                } else {
                    " WHERE scope <> 'reprocess'"
                }
            ),
        ))
        .await?;
        tx.execute(Statement::from_string(
            backend,
            "DROP TABLE store_reauth_grants_v056".to_string(),
        ))
        .await?;
        tx.execute(Statement::from_string(
            backend,
...
fn allowed_scopes(include_reprocess: bool) -> &'static str {
    if include_reprocess {
...
fn sqlite_table_sql(include_reprocess: bool) -> String {
    format!(
