...
function RoutingSettings({ form, setForm, settings, c }: { form: ProviderForm; setForm: React.Dispatch<React.SetStateAction<ProviderForm>>; settings?: SystemSettings; c: (zh: string, en: string) => string }) {
	return <div className='mx-auto flex w-full max-w-4xl flex-col gap-6 p-4 sm:p-6'><SectionHeading title={c('路由与重试', 'Routing and retries')} description={c('这些策略应用到 Provider 下的全部 Channel。', 'These policies apply to every channel in the provider.')} />
		<div className='grid gap-4 rounded-xl border bg-card p-4 sm:grid-cols-2 sm:p-5'>
			...
			<div className='flex items-center justify-between gap-4 rounded-lg border p-4'><div><Label>{c('带工具请求跳过此渠道', 'Skip tool-bearing requests')}</Label><p className='mt-1 text-xs text-muted-foreground'>{c('上游对带 tools 的请求不缓存时开启：此类请求路由到下一个 Provider。', 'Enable when the upstream does not cache tool-bearing requests: such requests route to the next provider.')}</p></div><Switch checked={form.prompt_cache_incompatible_with_tools ?? undefined} onCheckedChange={value => setForm(previous => ({ ...previous, prompt_cache_incompatible_with_tools: value }))} /></div>
		</div>
	</div>
}
