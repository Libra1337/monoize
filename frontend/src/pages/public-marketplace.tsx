...
function rangeLabel(range: MarketplaceRateRange | null): ReactNode {
  if (!range) return "—";
  const format = (value: string) => range.unit.toLowerCase() === "token"
    ? formatCoinPerMillionUsd(value)
    : formatCoinDecimalUsd(value, range.unit);
  if (range.min === range.max) return <CoinAmount value={format(range.min)} />;
  return <span className="inline-flex flex-wrap items-center gap-1"><CoinAmount value={format(range.min)} /><span aria-hidden="true">–</span><CoinAmount value={format(range.max)} /></span>;
}
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <Dialog open={selected !== null} onOpenChange={(open) => { if (!open) setSelected(null); }}>
        ...
        >
          ...
          {offersLoading ? <div className="space-y-3"><Skeleton className="h-28" /><Skeleton className="h-28" /></div> : offersError ? <Card className="p-4 text-destructive">{t("publicSite.marketplace.offersError")}</Card> : <div className="space-y-4">{offers?.offers.map((offer) => <Card key={`${offer.public_provider_name}:${offer.public_channel_name}`} className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">{offer.public_provider_name}</h3><p className="mt-1 text-sm text-muted-foreground">{offer.public_channel_name}</p></div><Badge variant="outline">{offer.api_type}</Badge></div><dl className="mt-4 divide-y border-y">{offer.rates.map((rate, index) => <div key={`${rate.usage_class}:${index}`} className="grid gap-1 py-3 text-sm sm:grid-cols-[minmax(0,1fr)_auto]"><dt>{rate.usage_class}{rate.context_tier ? ` · ${rate.context_tier}` : ""}{rate.service_tier ? ` · ${rate.service_tier}` : ""}{rate.modality ? ` · ${rate.modality}` : ""}{rate.cache_ttl ? ` · ${rate.cache_ttl}` : ""}</dt><dd className="font-mono text-xs"><CoinAmount value={rate.unit.toLowerCase() === "token" ? formatCoinPerMillionUsd(rate.display_rate_nano_usd) : formatCoinDecimalUsd(rate.display_rate_nano_usd, rate.unit)} /></dd></div>)}</dl></Card>)}</div>}
