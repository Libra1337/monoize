...
export function PublicMarketplacePage() {
  ...
  const { data, error, isLoading } = useSWR<MarketplaceResponse>(listKey, getJson, { keepPreviousData: true });
  const loadKey = loadCursor ? `/api/public/marketplace?${new URLSearchParams({ limit: "50", q: deferredSearch, cursor: loadCursor })}` : null;
  const { data: loadedPage, error: loadError, isLoading: isLoadingMore } = useSWR<MarketplaceResponse>(loadKey, getJson);
  ...
  useEffect(() => {
    if (!loadedPage || !loadCursor) return;
    setPages((current) => current.some((page) => page.revision === loadedPage.revision && page.items[0]?.model === loadedPage.items[0]?.model) ? current : [...current, loadedPage].slice(-3));
    setLoadCursor(null);
  }, [loadedPage, loadCursor]);
