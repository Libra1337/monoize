...
describe("Coin display", () => {
  ...
  test("converts wallet nano-USD balances through the CNY/USD snapshot", () => {
    expect(formatCoinFromNanoUsd("1000000000", "7.2")).toBe("¥7.20");
  });
  ...
  test("renders a server-normalized CNY rate as Coin without conversion", () => {
    expect(formatCoinPerUnitCny("1000", "token")).toBe("¥1.00 / 1M tokens");
    expect(formatCoinPerUnitCny("10000000", "token")).toBe("¥10000.00 / 1M tokens");
    expect(formatCoinPerUnitCny("1250000000.5", "call")).toBe("¥1.25 / call");
  });

  test("uses CNY as the Coin basis for CNY recharge products", () => {
    expect(formatCoinFromMinor("1234", "CNY", "7.2")).toBe("¥12.34");
  });

  test("converts USD recharge products to equivalent Coin at the quote rate", () => {
    expect(formatCoinFromMinor("1000", "USD", "7.2")).toBe("¥72.00");
  });

  test("changes Coin display values when the selected currency changes", () => {
    expect(formatCoinFromMinorForCurrency("720", "CNY", "CNY", "7.2")).toBe("¥7.20");
    expect(formatCoinFromMinorForCurrency("720", "CNY", "USD", "7.2")).toBe("$1.00");
    expect(formatCoinFromNanoUsdForCurrency("1000000000", "CNY", "7.2")).toBe("¥7.20");
    expect(formatCoinFromNanoUsdForCurrency("1000000000", "USD", "7.2")).toBe("$1.00");
  });
