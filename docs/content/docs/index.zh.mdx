...
- **路由与可靠性。** 一个逻辑模型可以匹配多个有序的 Provider（服务提供者）。每个 Provider 拥有一个 Channel。Monoize 重试可重试的失败，前进到下一个 Provider，并在发出第一个响应字节后停止回退。参见[路由与可靠性](/zh/docs/routing)。
- **变换（Transform）。** 36 个内置变换可按 Provider、API 密钥或全局范围修改请求和响应。参见[变换](/zh/docs/transforms)。
