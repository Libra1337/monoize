...
Monoize ships 35 built-in transforms. Each transform has its own page in this section.
...
| --- | --- | --- |
| Auto-cache | Improve provider-side prompt caching | `cache_anthropic_system`, `cache_anthropic_tool_use`, `cache_openai_prompt`, `cache_openai_tool_use`, `cache_prefix_stabilize`, `cache_user_id` |
| Field | Set, remove, or cap raw request/response fields | `field_override_max_tokens`, `field_remove`, `field_set`, `field_strip_sampling` |
