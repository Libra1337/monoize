...
The transform detects JPEG, PNG, and WebP from the image bytes. With `output_format = original`, it uses the detected format. When compression keeps the original bytes, it corrects a mismatched MIME type. Invalid images remain unchanged.
